...
inline int32 CLuaBaseEntity::rageEffectComplete(lua_State* L)
{
	ShowMessage("In RageEffectComplete");
	DSP_DEBUG_BREAK_IF(m_PBaseEntity == NULL);
    DSP_DEBUG_BREAK_IF(m_PBaseEntity->objtype != TYPE_PC);
	
	CStatusEffect * PEffect = new CStatusEffect(EFFECT_RAGED,EFFECT_MEDICINE,0,0,2700,0,0,0);
...
inline int32 CLuaBaseEntity::setXPRate(lua_State *L)
{
    DSP_DEBUG_BREAK_IF(m_PBaseEntity == NULL);
			CCharEntity* PChar = (CCharEntity*)m_PBaseEntity;
    if( !lua_isnil(L,1) && lua_isnumber(L,1) )
    {
        float xpRate = (float)lua_tonumber(L, 1);
        if (PChar->xpRate != xpRate)
		{
            PChar->xpRate = xpRate;
			PChar->pushPacket(new CCharUpdatePacket(PChar));
			return 0;
...
Lunar<CLuaBaseEntity>::Register_t CLuaBaseEntity::methods[] =
{
	...
	LUNAR_DECLARE_METHOD(CLuaBaseEntity,rageEffectComplete),
	LUNAR_DECLARE_METHOD(CLuaBaseEntity,setXPRate),
	{NULL,NULL}
};
